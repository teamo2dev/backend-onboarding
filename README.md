...
🙋‍♀️🙋‍♂️ : 맡은 작업을 완료했습니다, 코드 리뷰를 받고자 하는데 어떻게 하면 되나요 ?  
💁 : 저희는 GitHub 을 사용하고 있습니다 ! 여기에서 Pull Request 를 날려주신 뒤, 모든 인원을 리뷰어로 등록해주시면 됩니다.  
또한, PR 작성시에는 Label 이 필수 인데요, 어떤 Label 이냐 하면, 언제까지 리뷰가 완료되어야 한다는 의미를 지닌 Label 입니다.  
Label 은 `D-{n}` 의 형태로 되어 있으며 `n` 은 **완료 되어야 할 일 수** 를 의미합니다.  

```text
For Example,
D-0 : ASAP 리뷰로 빠르게 진행이 필요한 건들
D-2 : 보통의 리뷰이며, 이틀안에 진행이 되면 된다.
```

또한, 코드 리뷰의 사이즈, 즉, 작업의 사이즈는 작으면 작을 수록 좋습니다.  
그 이유는 짧은 호흡으로 작업을 가져갈 수도 있고, 해당 작업에 집중해서 작업도 가능할뿐더러, 리뷰해주는 사람도 빠르게 필요한 부분에 대해서 리뷰가 가능해집니다.  
큰 사이즈의 Pull Request 를 보면 숨이 턱 막혀버리죠..!  


**어느 정도가 작은걸까 할때에는, 이래도 되나 싶을 정도로 작게했을 때 보다 작게해주면 됩니다 !**

### 리뷰를 남기기

리뷰를 남겨주실 때는, 우선순위를 prefix 로 지정해서 입력해주시면 됩니다.  
거두 절미하고 예시를 들게요.

```text
p2) 여기는 이렇게 저렇게 바꿔보는건 어떨까요 ?
```

prefix 목록은 아래와 같습니다.
- p1: 꼭, 적극적으로 반영해주세요 (Request change)
- p2: 반영을 고려해봐주세요 (Request change)
- p3: 반영을 지금은 안해도 되나, **지금 반영안할 시 TODO 작성이라도 진행해주세요**. (Approve 가능)
- q : 질문이 필요한 경우
